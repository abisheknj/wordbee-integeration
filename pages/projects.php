...
function display_project_list() {
    ...
    $language_codes = [
        ...
        "es-CO", "es-CR", "es-CU", "es-DO", "es-EC" , "es-ES", "es-LA", "es-SV", "es-GQ", "es-GT", "es-HN", 
    ...
    if (isset($_POST['client_name']) || isset($_POST['date_from']) || isset($_POST['date_to']) || isset($_POST['src']) || isset($_POST['trg']) ) {
        ...
        $trg = isset($_POST['trg']) ? sanitize_text_field($_POST['trg']) : '';
            ...
            echo '<script>';
            echo 'var client_name = "' . esc_js($client_name) . '";';
            echo 'var src = "' . esc_js($src) . '";';
            echo 'var trg = "' . esc_js($trg) . '";';
            echo 'var date_from = "' . esc_js($date_from) . '";';
            echo 'var date_to = "' . esc_js($date_to) . '";';
            echo '</script>';
    ...
    if ($project_list !== false) {
        if (!empty($project_list)) {
            ...
            $output .= '<button class="export-button" onclick="exportTableToExcel(client_name,src, trg, date_from, date_to)">Export</button>';
            $output .= '</div>';
            ...
            $output .= '<th>Job Source</th>';
            $output .= '<th>Job Target</th>';
            $output .= '<th>Job Reference</th>';
            $output .= '<th>Word Count</th>';
            $output .= '<th>Char Count</th>';
            $output .= '<th>Edits</th>';
            $output .= '<th>Post Edit Distance</th>';
                ...
                if ($jobs && isset($jobs['rows']) && !empty($jobs['rows'])) {
                    foreach ($jobs['rows'] as $job) {
                        ...
                        if ($data !== false) {
                            ...
                            if ($decoded_data !== null && isset($decoded_data['counts']) && !empty($decoded_data['counts'])) {
                        ...
                        // $total_words = isset($total_word_counts['total_words']) ? $total_word_counts['total_words'] : 0;
                        // $total_chars = isset($total_word_counts['total_chars']) ? $total_word_counts['total_chars'] : 0;
    
                        $job_wordData = get_word_count_for_job($job['jobid']);
                        $word_count = isset($job_wordData['total_words']) ? $job_wordData['total_words'] : 0;
                        $char_count = isset($job_wordData['total_chars']) ? $job_wordData['total_chars'] : 0;
                        $post_edit_distance = ($word_count > 0) ? ($edits / $word_count) : 0;
                        if($word_count > 0){
                            $total_edit_distance += $post_edit_distance;
                            $job_count++;
                        }
                        ...
                        $output .= '<td>' . esc_html($job['jobid']) . '</td>';
                        ...
                        $output .= '<td>' . esc_html($post_edit_distance) . '</td>';
                        $output .= '</tr>';
            ...
            $avg_post_editdistance = ($job_count > 0) ? ($total_edit_distance / $job_count) : 0;
            $post_edit_distance_percentage = $avg_post_editdistance*100;
    
            $output .= '</table>';
            $output .= '<div class="card">';
            $output .= '<div class="card-body">';
            $output .= '<div class="edit-distance-info" title="Average Post Edit distance * 100 is calculated as Post Edit Distance Percentage">';
            $output .= '<span class="info-icon">ℹ️</span>';
            $output .= '</div>';
            $output .= '<h3 class="card-title">Average Post Edit Distance</h3>';
            $output .= '<p class="card-text">' . esc_html($avg_post_editdistance) . '</p>';
            $output .= '<h3 class="card-title">Post Edit Distance Percentage</h3>';
            $output .= '<p class="card-text">' . esc_html($post_edit_distance_percentage) . '%</p>';
            $output .= '</div>';
            $output .= '</div>';
